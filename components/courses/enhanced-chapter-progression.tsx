...
export const EnhancedChapterProgression = ({
  ...
  const getTotalItems = () => {
    return chapters.reduce(
      (total, chapter) =>
        total + 1 + chapter.quizzes.length + chapter.assignments.length,
      0
    );
  };
                    ...
                    {showContent &&
                      (chapter.quizzes.length > 0 ||
                        ...
                        <span className="text-xs text-gray-500">
                          •{" "}
                          {chapter.quizzes.filter((q) => q.isCompleted).length +
                            chapter.assignments.filter((a) => a.isCompleted)
                              .length +
                            (chapter.videoCompleted ? 1 : 0)}{" "}
                          of{" "}
                          {1 +
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {chapters.reduce(
                      (acc, ch) =>
                        acc +
                        ch.assignments.filter((a) => a.isCompleted).length,
                      0
                    )}
                    /
                    {chapters.reduce(
                      (acc, ch) => acc + ch.assignments.length,
                      0
                    )}
                  </span>
                </div>
