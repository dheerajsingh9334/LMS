...
const StudentDashboard = async () => {
  ...
  const totalChapters = purchases.reduce((total, purchase) => 
    total + purchase.course.chapters.length, 0
  );
  const completedChapters = userProgress.filter(progress => progress.isCompleted).length;
  const activeLiveSessions = purchases.reduce((total, purchase) => 
    total + purchase.course.liveSessions.length, 0
  );
  ...
  const recentCourses = purchases.slice(0, 3).map(purchase => {
    ...
    const courseCompleted = userProgress.filter(progress => 
      progress.isCompleted && 
      purchase.course.chapters.some(chapter => chapter.id === progress.chapterId)
    ).length;
    const courseProgress = courseChapters > 0 ? (courseCompleted / courseChapters) * 100 : 0;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">Welcome back, {user.name}! Continue your learning journey.</p>
        </div>
        ...
        <CardContent>
          {recentCourses.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-slate-600 mb-2">No courses enrolled</h3>
              <p className="text-slate-500 mb-4">Explore our course catalog and start learning!</p>
              <Button asChild>
                <Link href="/browse">
                  Browse Courses
                </Link>
