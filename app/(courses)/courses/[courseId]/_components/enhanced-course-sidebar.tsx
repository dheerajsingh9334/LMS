...
export const EnhancedCourseSidebar = async ({
  ...
  const totalItems = enhancedChapterData.reduce((total, chapter) => 
  ...
  const enhancedProgress = totalItems > 0 ? (completedItems / totalItems) * 100 : 0;
            ...
            {showContent ? (
              <>
                ...
                <div className="text-xs text-gray-500 grid grid-cols-2 gap-2">
                  <span>Chapters: {enhancedChapterData.filter(ch => ch.isCompleted).length}/{enhancedChapterData.length}</span>
                  <span>
                    Quizzes: {enhancedChapterData.reduce((acc, ch) => acc + ch.quizzes.filter(q => q.isCompleted).length, 0)}/
                    {enhancedChapterData.reduce((acc, ch) => acc + ch.quizzes.length, 0)}
                  </span>
                </div>
              </>
            ) : (
        ...
        {enhancedChapterData.map((chapter) => {
          return showContent && (purchased || isInstructor) ? (
            ...
            <CourseSidebarItem
              ...
              lockReason={chapter.isAccessible ? undefined : "Complete previous chapter first"}
            />
          );
        })}
      </div>
